...
* Extend the `topic-sender` so it sets a `number` property on each message that is incremented for each message. You can use an `AtomicLong` for incrementing the counter.
